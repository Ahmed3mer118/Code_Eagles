...
function FeedBack() {
  ...
  const nextFeedback = () => {
    if (currentIndex + itemsPerPage < feedbackList.length) {
  ...
  return (
    <>
      <Helmet>
        ...
        <meta
          name="description"
          content="See what our students say about their learning experience at Code Eagles"
        />
      </Helmet>
            ...
            <>
              ...
              <div className="flex justify-between items-center mt-8">
                ...
                <div className="text-sm text-gray-500">
                  Showing {Math.min(currentIndex + 1, feedbackList.length)}-
                  {Math.min(currentIndex + itemsPerPage, feedbackList.length)} of{" "}
                  {feedbackList.length}
                </div>

                <button
                  onClick={nextFeedback}
                  disabled={currentIndex + itemsPerPage >= feedbackList.length}
                  className={`flex items-center px-4 py-2 rounded-lg ${
                    currentIndex + itemsPerPage >= feedbackList.length
                      ? "text-gray-400 cursor-not-allowed"
                      : "text-blue-600 hover:bg-blue-50"
                  }`}
                >
