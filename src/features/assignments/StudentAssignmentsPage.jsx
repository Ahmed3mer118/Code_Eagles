              ...
              <article key={item._id} className="ce-card p-5">
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div>
                    ...
                    <p className="mt-1 text-sm text-[var(--ce-muted)]">{t('assignments.dueDate')}: {new Date(item.dueDate).toLocaleString()}</p>
                ...
                {!item.submission || item.submission.status !== 'graded' ? (
                  <button type="button" className="ce-btn ce-btn-accent mt-4 text-sm" onClick={() => { setActive(item); setFiles([]); setNotes(''); }}>
